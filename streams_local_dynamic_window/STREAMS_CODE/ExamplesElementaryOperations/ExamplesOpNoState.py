...
from Agent import *
from ListOperators import stream_operator_stateless
...
def example_1():
    ...
    # stream x.
    a = stream_operator_stateless(f, x)
    b = stream_operator_stateless(g, x)
    c = stream_operator_stateless(h, x)
    d = stream_operator_stateless(r, x)
    ...
    def F(x): return stream_operator_stateless(f,x)
    def G(x): return stream_operator_stateless(g,x)
    newa = F(x)
    newb = G(F(x))
...
def example_2():
    def f(x_list): return([v*v for v in x_list])
    def G(x_stream): return stream_operator_stateless(f, x_stream)
    def F(x_stream, list_of_trigger_streams):
        return stream_operator_stateless(f, x_stream, call_streams=list_of_trigger_streams)
    x_stream = Stream('x')
    trigger_stream = Stream('wake up')
    list_of_trigger_streams = [trigger_stream]
    y_stream = F(x_stream, list_of_trigger_streams)
    z_stream = G(x_stream)
    ...
    print "calling trigger_stream"
    trigger_stream.extend([1])
    print_stream_recent(x_stream)
    print_stream_recent(y_stream)
    print_stream_recent(z_stream)
    ...
    x_stream = Stream('x')
    trigger_stream = Stream('trigger_stream')
    list_of_trigger_streams = [trigger_stream]
    y_stream = F(x_stream, list_of_trigger_streams)
    y_stream.set_name('y')

    x_stream.extend(range(2, 20, 4))
    print_stream_recent(x_stream)
    print_stream_recent(y_stream)
    print 'trigger'
    trigger_stream.extend([1])
